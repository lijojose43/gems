...
class StorageManager {
    ...
    importData(data) {
        try {
            ...
            this.updateSetting(this.STORAGE_KEYS.LAST_BACKUP, new Date().toISOString());
            return true;
        } catch (error) {
    ...
    getStats() {
        ...
        const recentCalls = communication.filter(c => 
            c.type === 'call' && new Date(c.date) >= thirtyDaysAgo
        ).length;

        const pendingFollowups = communication.filter(c => {
            const commDate = new Date(c.date);
            const daysSinceContact = Math.floor((now - commDate) / (1000 * 60 * 60 * 24));
