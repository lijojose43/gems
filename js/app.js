...
class GemsApp {
    ...
    setupEventListeners() {
        ...
        // Communication form
        document.getElementById('communication-form').addEventListener('submit', (e) => {
            e.preventDefault();
        ...
        document.getElementById('export-communication-btn').addEventListener('click', () => {
        ...
        document.getElementById('communication-filter').addEventListener('change', (e) => {
    ...
    updateDashboard() {
        ...
        // Update stat cards
        document.getElementById('total-students').textContent = ui.formatNumber(stats.totalStudents);
        document.getElementById('recent-calls').textContent = ui.formatNumber(stats.recentCalls);
        document.getElementById('pending-followups').textContent = ui.formatNumber(stats.pendingFollowups);
    ...
    handleStudentSubmit() {
        ...
        if (this.currentEditId) {
            ...
            const updatedStudent = storage.updateStudent(this.currentEditId, formData);
            if (updatedStudent) {
                ui.showToast('Student updated successfully', 'success');
                this.logActivity('student_updated', `Updated student: ${updatedStudent.name}`);
            } else {
            ...
            if (newStudent) {
                ...
                this.logActivity('student_added', `Added new student: ${newStudent.name}`);
            } else {
        ...
        ui.confirm(
            ...
            () => {
                if (storage.deleteStudent(studentId)) {
                    ...
                    this.logActivity('student_deleted', `Deleted student: ${student.name}`);
                    this.renderStudents();
                    this.updateDashboard();
                } else {
    ...
    renderCommunication(filterType = 'all') {
        ...
        communication.sort((a, b) => new Date(b.date + ' ' + b.time) - new Date(a.date + ' ' + a.time));
        
        if (communication.length === 0) {
            container.innerHTML = '<p class="text-muted">No communication logs found</p>';
            return;
        }
    ...
    openCommunicationModal() {
        ...
        document.getElementById('comm-time').value = new Date().toTimeString().slice(0, 5);
    ...
    handleCommunicationSubmit() {
        ...
        if (newLog) {
            ...
            this.logActivity(formData.type, `${formData.type} with ${student ? student.name : 'Unknown Student'}`);
    ...
    callStudent(mobileNumber) {
        ...
        // Log the call
        const students = storage.getStudents();
        const student = students.find(s => s.mobile1 === mobileNumber || s.mobile2 === mobileNumber);
    ...
    whatsappStudent(mobileNumber) {
        ...
        window.open(`https://wa.me/${mobileNumber.replace(/[^\d]/g, '')}?text=${message}`, '_blank');
    }
    ...
    exportCommunication() {
        try {
            ...
            const students = storage.getStudents();
            const csvContent = csvManager.exportCommunication(communication, students);
